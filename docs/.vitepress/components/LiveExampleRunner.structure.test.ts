...
describe('LiveExampleRunner structure', () => {
  ...
  it('groups preview, source toolbar, source code and collapsed editor into one Element Plus style example card', () => {
    const runner = readFileSync('docs/.vitepress/components/LiveExampleRunner.vue', 'utf8')
    ...
    const toolbarIndex = runner.indexOf('<LiveExampleToolbar', cardStart)
    ...
    const cardEnd = runner.indexOf('</section>', editorIndex)

    expect(cardStart).toBeGreaterThan(-1)
    expect(cardStart).toBeLessThan(gridIndex)
    expect(gridIndex).toBeLessThan(previewIndex)
    expect(previewIndex).toBeLessThan(toolbarIndex)
    expect(toolbarIndex).toBeLessThan(sourceIndex)
    expect(sourceIndex).toBeLessThan(editorDisclosureIndex)
    expect(editorDisclosureIndex).toBeLessThan(editorIndex)
    expect(editorIndex).toBeLessThan(cardEnd)
    expect(runner).toMatch(/<details class="live-example-runner__editor-disclosure">\s*<summary/)
    expect(css).toMatch(/\.live-example-runner__example-card \.live-example-runner__preview\s*\{[^}]*border:\s*0;/)
    expect(css).toMatch(/\.live-example-runner__toolbar\s*\{[^}]*min-height:\s*48px;/)
  })
  ...
  it('places the real editable example before scenario matrix, inspector and workflow diagnostics', () => {
    ...
    expect(cardIndex).toBeLessThan(toolbarIndex)
    expect(toolbarIndex).toBeLessThan(scenarioIndex)
