...
::: tip TIP
`YCascaderPanel` 文档已按 Select 页面同一套结构组织：每个场景独立成段，示例块保留 TS/JS 切换、复制代码和展开源码，API 与可访问性约定集中在页尾。
:::

## Embedded taxonomy {#cascader-panel-embedded-taxonomy}
...
## Multiple scopes {#cascader-panel-multiple-scopes}
...
## Lazy panel {#cascader-panel-lazy-panel}
...
## Cascader Panel API {#cascader-panel-api}
