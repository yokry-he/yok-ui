...
::: tip TIP
`YCascader` 文档已按 Select 页面同一套结构组织：每个场景独立成段，示例块保留 TS/JS 切换、复制代码和展开源码，API 与可访问性约定集中在页尾。
:::

## Basic path {#cascader-basic-path}
...
## Disabled branch {#cascader-disabled-branch}
...
## Multiple leaf paths {#cascader-multiple-leaf-paths}
...
## Custom option slot {#cascader-custom-option-slot}
...
## Async loading {#cascader-async-loading}
...
## Usage notes {#cascader-usage-notes}
