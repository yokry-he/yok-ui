...
::: tip TIP
`YColorPickerPanel` 文档已按 Select 页面同一套结构组织：每个场景独立成段，示例块保留 TS/JS 切换、复制代码和展开源码，API 与可访问性约定集中在页尾。
:::

## Embedded theme color {#color-picker-panel-embedded-theme-color}
...
## Brand presets {#color-picker-panel-brand-presets}
...
## Alpha token {#color-picker-panel-alpha-token}
...
## Panel validation {#color-picker-panel-panel-validation}
...
## Usage notes {#color-picker-panel-usage-notes}
...
## Color Picker Panel API {#color-picker-panel-api}
