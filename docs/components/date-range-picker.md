...
::: tip TIP
`YDateRangePicker` 文档已按 Select 页面同一套结构组织：每个场景独立成段，示例块保留 TS/JS 切换、复制代码和展开源码，API 与可访问性约定集中在页尾。
:::

## Basic range {#date-range-picker-basic-range}
...
## Range shortcuts {#date-range-picker-range-shortcuts}
...
## Disabled dates {#date-range-picker-disabled-dates}
...
## Form validation {#date-range-picker-form-validation}
...
## Usage notes {#date-range-picker-usage-notes}
...
## Live workflow {#date-range-picker-live-workflow}
...
## Date Range Picker API {#date-range-picker-api}
