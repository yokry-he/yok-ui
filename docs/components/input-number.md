...
::: tip TIP
`YInputNumber` 文档已按 Select 页面同一套结构组织：每个场景独立成段，示例块保留 TS/JS 切换、复制代码和展开源码，API 与可访问性约定集中在页尾。
:::

## Bounded quantity {#input-number-bounded-quantity}
...
## Precision {#input-number-precision}
...
## Without controls {#input-number-without-controls}
...
## Error state {#input-number-error-state}
...
## Usage notes {#input-number-usage-notes}
...
## Input Number API {#input-number-api}
