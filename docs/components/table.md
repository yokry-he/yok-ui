...
::: tip TIP
`YTable` 文档已按 Select 页面同一套结构组织：每个场景独立成段，示例块保留 TS/JS 切换、复制代码和展开源码，API 与可访问性约定集中在页尾。
:::

## Selectable table {#table-selectable-table}
...
## Expandable rows {#table-expandable-rows}
...
## Loading and summary {#table-loading-and-summary}
...
## Fixed columns {#table-fixed-columns}
...
## Resizable columns {#table-resizable-columns}
...
## Virtualized rows {#table-virtualized-rows}
...
## Custom empty state {#table-custom-empty-state}
