...
::: tip TIP
`YVirtualizedSelect` 文档已按 Select 页面同一套结构组织：每个场景独立成段，示例块保留 TS/JS 切换、复制代码和展开源码，API 与可访问性约定集中在页尾。
:::

## Large option set {#virtualized-select-large-option-set}
...
## Filterable {#virtualized-select-filterable}
...
## Multiple {#virtualized-select-multiple}
...
## Disabled {#virtualized-select-disabled}
...
## Usage notes {#virtualized-select-usage-notes}
...
- 当前版本复用 `YSelect` 的过滤、远程搜索和加载状态逻辑；远程大列表优先使用 `remoteMethod`，`search` 事件可继续用于日志或外部状态复现。
- 选项分组和 allow-create 属于普通 `YSelect` 的能力；虚拟化场景优先保持扁平列表，保证滚动计算稳定。
- 多选时建议同时开启 `collapseTags`，避免触发器被大量已选标签撑开。

## Virtualized Select API {#virtualized-select-api}
