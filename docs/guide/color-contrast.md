...
主题测试会检查 `yok-light`、`yok-clean` 和 `yok-candy` 的关键组合：
...
新增主题时，把主题加入 `builtinThemes` 和 `packages/themes/src/tokens.test.ts` 的对比度矩阵。若某个颜色只用于装饰，不应进入文字或按钮前景/背景组合；若它会承载文本，就必须通过 `4.5:1`。
