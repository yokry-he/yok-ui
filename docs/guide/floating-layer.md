...
| `YTimeSelect` | 固定时间列表 combobox / listbox 面板 |
...
组件仍然负责自己的语义、焦点和事件，例如 `YDropdown` 维护 menu 键盘导航，`YSelect` / `YTimeSelect` 维护 combobox / listbox 选择逻辑，`YDatePicker` 维护 grid 键盘导航，`YTooltip` 维护 `aria-describedby`。`useFloatingLayer` 只负责可靠定位，`useDismissableLayer` 只负责通用关闭判断，`useLayerStack` 只负责层级顺序，三者都不接管组件业务状态。
