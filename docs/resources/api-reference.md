...
Yok UI 的 API 数据中心把组件文档从“手写表格”升级为结构化数据。当前 81 个注册组件都已接入 `componentRegistry` 的 props、events、slots 或 types 数据；组件页、包页、搜索页和后续 Playground 都应复用同一份来源，避免 API 说明漂移。
